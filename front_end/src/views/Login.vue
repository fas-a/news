    ...
    try {
        ...
        if (!response.ok) {
            if(response.status === 401) {
                ...
                showError("Login failed", "An unexpected error occurred. Please try again later.");
            }
        } else {
            const data = await response.json();
            if (data.token) {
...
<template>
  <div class="layout-grid login-panel">
    <Panel>
        ...
        <h3 style="display: flex; flex-direction: column; align-items: center;justify-content: center;">Login</h3>
